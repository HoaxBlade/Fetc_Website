...
import { 
  ...
  Save, Sparkles, User, MapPin, Phone, Calendar, Upload, ShieldAlert, X, 
  MessageSquare, BookOpen, FileCheck
} from 'lucide-react';
...
function UserDoubts() {
  ...
  if (isDoubtsLoading || (activeTab === "documentation" && isFetchingLead)) {
    return (
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-100 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-slate-900 via-indigo-950 to-indigo-800 bg-clip-text text-transparent">
            Student Support & Onboarding Hub
          </h1>
          <p className="text-slate-500 font-medium text-sm mt-1.5">
            Resolve academic questions or complete your verification dossier in one place.
          </p>
        </div>
      </div>

      {/* Tab Switcher */}
      <div className="flex bg-slate-100/80 backdrop-blur-md p-1.5 rounded-2xl gap-2 mb-10 w-fit border border-slate-200/40">
        <button
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ----------------- 2. DOCUMENTATION PORTAL TAB ----------------- */}
        {activeTab === "documentation" && (
          <motion.div
            key="documentation-tab"
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.2 }}
